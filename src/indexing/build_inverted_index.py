...
def build_inverted_index(tokenized_docs: List[Dict[str, List[str]]]) -> Dict[str, List[int]]:
    inverted_index = defaultdict(list)
    
    for doc in tokenized_docs:
        doc_id = doc["id"]
        tokens = doc["tokens"]
        for token in set(tokens):  # Using set to avoid duplicate entries for the same document
...
if __name__ == "__main__":
    ...
    logger.info(f"Loaded inverted index with {len(fetch_inverted_index(inverted_index_file))} terms")