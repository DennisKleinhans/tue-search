...
from preprocessing import PreprocessingModule, get_vocab_encoding
...
def FF_embed_batch(batch):
    embeddings = [
        [get_vocab_encoding(token, vocab) for token in document][:model_config.max_position_embeddings]  # truncation
...
def SDE_embed_batch(batch):
    qry_embeddings = [
        [get_vocab_encoding(token, vocab) for token in query][:model_config.max_position_embeddings]  # truncation
...
if LOAD_DATASET_FROM_DISK:
    embedded_dataset = load_from_disk(DATASET_SAVE_PATH+f"{pipeline_config.model}-embedded_dataset_bs{training_config.batch_size}")
    print(" - loaded from disk")
else:
    embedded_dataset = preprocessed_dataset.map(
        EMBEDDING[pipeline_config.model], 
        batched=True
    )
    embedded_dataset.save_to_disk(DATASET_SAVE_PATH+f"{pipeline_config.model}-embedded_dataset_bs{training_config.batch_size}")
    print(" - done")
...
if pipeline_config.model == "FF":
    ...
    raise ValueError(f"Model {pipeline_config.model} is not implemented!")
print(" - done")
...
print(f"training {pipeline_config.model}...")
if pipeline_config.model == "FF":
