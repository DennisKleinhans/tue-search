...
class FastSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.softmax = self._ScaledSoftmax

        self.apply(self.init_weights)

    def _ScaledSoftmax(self, logits):
...
class SDE(torch.nn.Module):
    ...
    def forward(self, qry_input_ids, doc_input_ids, targets):
        # query processing
        qry_mask = qry_input_ids.bool().float()
        qry_embds = self.word_embedding(qry_input_ids)
        qry_rep = self.shared_fastformer_encoder(qry_embds, qry_mask)

        # document processing
        doc_mask = doc_input_ids.bool().float()
        doc_embds = self.word_embedding(doc_input_ids)
