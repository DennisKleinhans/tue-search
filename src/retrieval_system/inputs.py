...
def get_feature_columns(mode):
    ...
    if mode == slim.learn.ModeKeys.TRAIN:
    ...
    if mode == slim.learn.ModeKeys.EVAL:
...
def create_input_fn(mode, input_files, batch_size, num_epochs):
    def input_fn():
        features = slim.layers.create_feature_spec_for_parsing(
            get_feature_columns(mode)
        )

        feature_map = slim.learn.io.read_batch_features(
